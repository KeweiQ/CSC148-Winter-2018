"""
unit tests for list_odd
"""
import unittest
from hypothesis import given
from hypothesis.strategies import integers
from hypothesis.strategies import lists
from nested_list_lab import list_odd
...
class ListOddTests(unittest.TestCase):
    def test_returns_list(self):
        """
        Test list_odd to make sure it returns a list.
        """
        return_type = type(list_odd(1))
        self.assertEqual(return_type, list, "list_odd should return type " +
                         "list, but instead returned type {}.".format(
                             return_type))

    def test_even_integer(self):
        """
        Test list_odd on an even integer.
        """
        self.assertEqual(list_odd(2), [], "list_odd should return an empty " +
                         "list when passed an even integer.")

    def test_odd_integer(self):
        """
        Test list_odd on an odd integer.
        """
        self.assertEqual(list_odd(1), [1], "list_odd should return a " +
                         "list containing the odd integer passed in when " +
                         "given an odd integer.")

    def test_one_nested_sublist(self):
        """
        Test list_odd on a list with only one sublist in it.
        """
        self.assertEqual(list_odd([[1]]), [1], "list_odd should return " +
                         "the odd integers in nested sublists.")


    def test_one_nested_sublist_and_integers(self):
        """
        Test list_odd on a list with at most one level of nested sublists in
        it mixed with integers.
        """
        self.assertEqual(list_odd([[1], 2, [5]]), [1, 5],
                         "list_odd should return the odd integers in nested" +
    ...
    @given(lists(integers(min_value = 0, max_value = 100), max_size = 20),
           lists(integers(min_value = 0, max_value = 20), min_size = 1,
                 max_size = 20))
    def test_list_odd(self, list_values, list_indices):
        """
        Test the submitted list_odd against a randomly generated list.
        """
