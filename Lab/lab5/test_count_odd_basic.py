"""
count_odd unit tests
"""
import unittest
from hypothesis import given
from hypothesis.strategies import integers
from hypothesis.strategies import lists
from nested_list_lab import count_odd
...
class CountOddTests(unittest.TestCase):
    def test_returns_int(self):
        """
        Test list_odd to make sure it returns an int.
        """
        return_type = type(count_odd(1))
        self.assertEqual(return_type, int, "count_odd should return type " +
    ...
    def test_even_integer(self):
        ...
        """
        self.assertEqual(count_odd(2), 0, "count_odd should return 0 " +
                         "when passed an even integer.")

    def test_odd_integer(self):
        """
        Test count_odd on an odd integer.
        """
        self.assertEqual(count_odd(3), 1, "count_odd should return 1 " +
                         "when given an odd integer.")

    def test_one_nested_sublist(self):
        """
        Test count_odd on a list with only one sublist in it.
        """
        self.assertEqual(count_odd([[3]]), 1, "count_odd should return " +
    ...
    def test_one_nested_sublist_and_integers(self):
        ...
        """
        self.assertEqual(count_odd([[3], 4, [5]]), 2,
                         "count_odd should return the number of odd " +
