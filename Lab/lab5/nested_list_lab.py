...
def max_length(obj: Union[list, object]) -> int:
    ...
    if not isinstance(obj, list):
        ...
        return max([len(obj)] + [max_length(x) for x in obj])
...
def list_over(obj: Union[list, str], n: int) -> List[str]:
    ...
    if isinstance(obj, str) and len(obj) > n:
        return [obj]
    elif isinstance(obj, str) and len(obj) <= n:
        return []
    else:
...
def count_above(obj: Union[list, int], n: int) -> int:
    ...
    2
    """
    if not isinstance(obj, list) and obj > n:
        return 1
    elif not isinstance(obj, list) and obj <= n:
        return 0
    else:
