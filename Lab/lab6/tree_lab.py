...
def sum_leaves(t: Tree) -> int:
    ...
    if t.children == []:
        ...
        return sum([sum_leaves(c) for c in t.children])
...
def contains_test_passer(t: Tree, test: Callable[[Any], bool]) -> bool:
    ...
    True
    """
    return test(t.value) or any([contains_test_passer(c, test) for c in t.children])
...
def list_if(t: Tree, p: Callable[[Any], bool]) -> list:
    ...
    """
    if p(t.value):
        return [t.value] + sum([list_if(c, p) for c in t.children], [])
    else:
        return sum([list_if(c, p) for c in t.children], [])
