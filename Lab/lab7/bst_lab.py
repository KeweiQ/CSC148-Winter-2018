...
def list_longest_path(node: Union[BTNode, None]) -> list:
    ...
    if node is None:
        ...
        return [node.data] + (left if len(left) >= len(right) else right)
...
def list_between(node: Union[BTNode, None], start: object, end: object) -> list:
    ...
    if node is None:
        ...
        r.append([node.data])
        r.append(list_between(node.right, start, end))
        r.append(list_between(node.left, start, end))
...
def sum_leaves(t: Union[BTNode, None]) -> int:
    """
    Return the sum of the values in the leaves of BinaryTree t.
    Return 0 if t is empty.
...
def sum_internal(t: Union[BTNode, None]) -> int:
    ...
    if t is None or (t.left is None and t.right is None):
        ...
        return sum([t.data, sum_internal(t.left), sum_internal(t.right)])
